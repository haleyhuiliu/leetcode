...
var maxSumTwoNoOverlap = function(A, L, M) {
    ...
    const sums = new Array(len).fill(null);
    
    for (let i = 0; i < len; i++) {
        sums[i] = new Array(len).fill(0);
    }
    ...
    for (let i = 0; i + L + M - 1 < len; i++) { 
        for (let j = i + L; j + M - 1 < len; j++ ) {
            let sum1 = sums[i][i + L - 1];
            let sum2 = sums[j][j + M - 1];
            sum = Math.max(sum, sum1 + sum2);
        }
        
        for (let j = i + M; j + L - 1 < len; j++) {
            let sum1 = sums[i][i + M - 1];
            let sum2 = sums[j][j + L - 1];
